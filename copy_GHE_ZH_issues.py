...
def read_issues(state):
    ...
    issues = gh.makeCall('%s/issues' % source_repo_url, ghe_headers, parameters,
                          print_status=True)
    issues_to_include = []
...
def write_issues(issues):
    ...
    target_milestones = gh.makeCall('%s/milestones' % target_repo_url, ghe_headers, {})
    issue_map = {}
    for issue in issues:
        target_assignees = []
...
def close_target_issues(issues, issue_map):
    ...
    source_milestones_url = '%s/milestones' % source_repo_url
    target_milestones_url = '%s/milestones' % target_repo_url
    source_milestones = gh.makeCall(source_milestones_url, ghe_headers, {'state': 'all'})
    target_milestones = gh.makeCall(target_milestones_url, ghe_headers, {'state': 'all'})
